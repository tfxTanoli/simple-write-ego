...
const UserManagementPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setFilteredUsers(users.filter(u => 
      u.name.toLowerCase().includes(term) || u.email.toLowerCase().includes(term) || u.id.includes(term)
    ));
  }, [searchTerm, users]);
  ...
  const handleStatusToggle = (user: User) => {
    ...
    // Use a simpler confirmation or custom UI in real app, standard confirm for now
    if (confirm(`Are you sure you want to ${newStatus === 'suspended' ? 'suspend' : 'activate'} ${user.name}?`)) {
      updateUserInDb({ ...user, status: newStatus });
      loadUsers();
    }
  };
  ...
  const handleSave = () => {
    if (editingUser) {
      updateUserInDb({ ...editingUser, ...formData } as User);
      setEditingUser(null);
      loadUsers();
    }
  };
  ...
  const handleDeleteUser = () => {
    if (editingUser) {
        if(confirm(`CRITICAL WARNING: This will permanently delete ${editingUser.name}. This action cannot be undone. Confirm delete?`)) {
            // In a real app, delete from DB. Here we mock by just closing and removing locally.
            // Note: storageService doesn't have deleteUser exposed, we'll just mock status update to 'deleted' or filter it out.
            // For this demo, we'll mark as suspended and rename to imply deletion or just close modal.
            updateUserInDb({ ...editingUser, status: 'suspended', name: `${editingUser.name} (Deleted)` });
            setEditingUser(null);
            loadUsers();
        }
    }
                  ...
                  <tr key={user.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition duration-150">
                    ...
                    <td className="p-4">
                      <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold border ${
                        user.plan === PlanType.FREE ? 'bg-slate-100 border-slate-200 text-slate-600' : 
